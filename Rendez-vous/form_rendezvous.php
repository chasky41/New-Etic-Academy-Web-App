    ...
    <div class="container mt-5">
        ...
        <form id="rendezvous-form" action="process_rendezvous.php" method="POST">
            ...
            <div class="mb-3">
                ...
                <div class="form-text" style ="color : red">Les rendez-vous doivent être pris entre juin et octobre. Les rendez-vous en dehors de cette période seront annulés.06/07/08/09/10</div>
            </div>
            <div class="mb-3">
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            form.addEventListener("submit", function (event) {
                ...
                var selectedHeure = heureInput.value;

                // Vérifier la disponibilité du rendez-vous via AJAX
                var xhr = new XMLHttpRequest();
                xhr.open("GET", "check_availability.php?date=" + encodeURIComponent(selectedDate) + "&heure=" + encodeURIComponent(selectedHeure), true);
                xhr.onreadystatechange = function () {
                    if (xhr.readyState === XMLHttpRequest.DONE) {
                        if (xhr.status === 200) {
                            var response = JSON.parse(xhr.responseText);
                            if (response.available) {
                                // Les données sont disponibles, envoyer les données via EmailJS
                                var formData = {
                                    nom: form.nom.value,
                                ...
                                emailjs.send("service_azs016e", "template_d8h7gam", formData)
                                    .then(function(response) {
                                        ...
                                        form.submit(); // Soumettre le formulaire après l'envoi par EmailJS
                                    }, function(error) {
                                        console.log("FAILED...", error);
                                    });
                            } else {
                                alert("Cette date et heure sont déjà réservées. Veuillez choisir une autre date ou heure.");
                            }
                        } else {
                            alert("Une erreur est survenue lors de la vérification de disponibilité.");
                        }
                    }
                };
                xhr.send();
